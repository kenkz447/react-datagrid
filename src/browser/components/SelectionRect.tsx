...
function SelectionRectImpl({
    ...
    const selectionRect = selection && {
        ...
        height:
            getRowSize(selection.max.row).top +
    ...
    const expandRowsIndicator = maxSelection &&
        ...
        top:
            getRowSize(maxSelection.row).top +
            getRowSize(maxSelection.row).height +
            headerRowHeight,
        transform: `translate(-${maxSelection.col <
            columnWidths.length - (hasStickyRightColumn ? 3 : 2)
            ? 50
            : 100
        }%, -${maxSelection.row < data.length - 1 ? 50 : 100}%)`,
    };
    ...
    const expandRowsRect = minSelection &&
        ...
        height:
            getRowSize(maxSelection.row + expandSelection).top +
    ...
    return (
        <>
            <div
                ...
                style={{
                    height:
                        getRowSize(data.length - 1).top +
                        getRowSize(data.length - 1).height +
                        headerRowHeight,
